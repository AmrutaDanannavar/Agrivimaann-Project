...
const Services = () => {
  ...
  return (
    <div>
        ...
        <div className=' flex flex-col md:flex-row gap-20 pl-7  pt-14 pb-4'>
          {cards.map((card) => (
            <div className='  w-72 h-72 bg-color4 relative hover:scale-110 duration-300 rounded'>
              ...
              <p className='text-white text-center text-sm p-2'>{card.desc}</p>
            </div>
          ))}
        </div>
      </div>

      <div className=' flex flex-col md:flex-row gap-10 bg-color1 h-auto pb-4'>
      ...
      <div className='bg-color1 h-auto'>
        <h1 className='text-black text-xl text-center pt-7'>BENEFITS</h1>
        <h1 className='text-black text-2xl text-center pt-4' >Embrace the Benefits of
          Drones for</h1>
        <h1 className='text-black text-2xl text-center '>Enhanced Efficiency and Higher Yields</h1>
        ...
        <div className='flex flex-col  md:flex-row justify-center gap-20 pt-7 pb-4'>
          {
            card2.map((card) => (
              <div className='w-64 md:h-52 h-52 pt-4 md:pl-0 pl-28 '>
                ...
                <h1 className='text-black text-center  text-xl bold pr-4 md:pr-14 pt-4'>{card.name}</h1>
